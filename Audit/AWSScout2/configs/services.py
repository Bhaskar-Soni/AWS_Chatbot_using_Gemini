...
class ServicesConfig(object):
    ...
    def __init__(self, metadata, thread_config = 4):
        ...
        self.ses = SESConfig(metadata['message']['ses'], thread_config)
        self.sns = SNSConfig(metadata['message']['sns'], thread_config)
        self.sqs = SQSConfig(metadata['message']['sqs'], thread_config)
